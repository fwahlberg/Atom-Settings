...
function getRootDeps(template, context) {
    if (production && template._V_) {
        return template._V_;
    }

    attachDepsAndComponentsToTemplate(template, context);

    var deps = template._V_ = Object.keys(template._W_).map(key => template._W_[key]);
    var initModule = getInitModule(template.path, template._s_);
...
function attachDepsAndComponentsToTemplate(target, context) {
    ...
    if (production && target._W_) return;

    var deps = target._W_ = {};
    var components = target._s_ = {};
    ...
    if (meta.tags) {
        meta.tags.forEach(tagPath => {
            ...
            if (tag._W_) Object.assign(deps, tag._W_);
            if (tag._s_) Object.assign(components, tag._s_);
        });
    }
}
