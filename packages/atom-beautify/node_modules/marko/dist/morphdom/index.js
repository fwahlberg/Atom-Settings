...
var VElement = require("../runtime/vdom/vdom").an_;
var virtualizeElement = VElement.ao_;
var morphAttrs = VElement.ap_;
...
function morphdom(parentNode, startNode, endNode, toNode, doc, componentsContext) {
    ...
    function insertVirtualNodeBefore(vNode, key, referenceEl, parentEl, component) {
        var realNode = vNode.ar_(doc);
        insertBefore(realNode, referenceEl, parentEl);

        if (vNode.as_ === ELEMENT_NODE) {
            if (key) {
                realNode.ai_ = key;
    ...
    function resolveComponentEndNode(startNode, vChild, parentNode) {
        ...
        // a single text node
        var isPrevText = vChild.as_ === TEXT_NODE;

        while (vChild = vChild.at_) {
            ...
            var isText = vChild.as_ === TEXT_NODE;
    ...
    function morphComponent(parentFromNode, component, vComponent) {
        ...
        // can be repeated
        component.x_ = globalComponentsContext._B_();
        ...
        if (startNode.au_ !== undefined || startNode.B_) {
            startNode = insertBefore(createMarkerComment(), startNode, parentFromNode);
        }

        if (endNode.au_ !== undefined || endNode.i_) {
    ...
    function morphChildren(parentFromNode, startNode, endNode, toNode, component) {
        ...
        outer: while (curToNodeChild) {
            ...
            curToNodeType = curToNodeChild.as_;

            var componentForNode = curToNodeChild._b_ || component;

            if (curToNodeType === COMPONENT_NODE) {
                if ((matchingFromComponent = existingComponentLookup[componentForNode.id]) === undefined) {
                    if (isRerenderInBrowser === true) {
                        var firstVChild = curToNodeChild.av_;
                        if (firstVChild) {
                            if (!curFromNodeChild) {
                    ...
                    if (matchingFromComponent.h_ !== curFromNodeChild) {
                        if (curFromNodeChild && (fromComponent = curFromNodeChild.B_) && globalComponentsContext._A_[fromComponent.id] === undefined) {
                ...
                continue;
            } else if (curToNodeKey = curToNodeChild.ax_) {
                curVFromNodeChild = undefined;
                curFromNodeKey = undefined;

                var keySequence = componentForNode.x_ || (componentForNode.x_ = globalComponentsContext._B_());
                ...
                if (curFromNodeChild) {
                    if (curFromNodeChild !== endNode) {
                        curFromNodeKey = curFromNodeChild.ai_;
                        curVFromNodeChild = curFromNodeChild.aj_;
                ...
                if (curFromNodeKey === curToNodeKey) {
                    ...
                    if ((matchingFromEl = componentForNode.w_[curToNodeKey]) === undefined) {
                        if (isRerenderInBrowser === true && curFromNodeChild && curFromNodeChild.nodeType === ELEMENT_NODE && curFromNodeChild.nodeName === curToNodeChild.aq_) {
                            curVFromNodeChild = virtualizeElement(curFromNodeChild);
                            curFromNodeChild.ai_ = curToNodeKey;
                            morphEl(curFromNodeChild, curVFromNodeChild, curToNodeChild, componentForNode, curToNodeKey);
                            curToNodeChild = toNextSibling;
                        ...
                        if (matchingFromEl.au_ !== undefined) {
                            matchingFromEl.au_ = undefined;
                        }
                        curVFromNodeChild = matchingFromEl.aj_;
                        ...
                        if (compareNodeNames(curVFromNodeChild, curToNodeChild)) {
                            if (fromNextSibling === matchingFromEl) {
                                ...
                                if (toNextSibling && toNextSibling.ax_ === curFromNodeKey) {
                                    // Single element swap
            ...
            while (curFromNodeChild && curFromNodeChild !== endNode) {
                ...
                if (curFromNodeType === curToNodeType) {
                    if (curFromNodeType === ELEMENT_NODE) {
                        ...
                        } else if (curFromNodeKey = curVFromNodeChild.ax_) {
                        ...
                        if (content == curToNodeChild.ay_) {
                            if (/^F\^/.test(content)) {
                                ...
                                while (curToNodeChild = curToNodeChild.at_) {
                                    if (curToNodeChild.ay_ === closingContent) {
                                        break;
                                    }
                                }
                                curToNodeChild = curToNodeChild.at_;
                                curFromNodeChild = curFromNodeChild === endNode ? null : curFromNodeChild.nextSibling;
                                continue outer;
                            }
                        } else {
                            curFromNodeChild.nodeValue = curToNodeChild.ay_;
                        }
                    }
                }
        ...
        while (curFromNodeChild && (endNode === null || curFromNodeChild !== endNode)) {
            ...
            fromComponent = curVFromNodeChild && curVFromNodeChild._b_ || component;
    ...
    function morphEl(fromEl, vFromEl, toEl, component, toElKey) {
        var nodeName = toEl.aq_;

        if (isRerenderInBrowser === true && toElKey) {
            component.w_[toElKey] = fromEl;
        }

        var constId = toEl.az_;
        if (constId !== undefined && vFromEl.az_ === constId) {
            return;
        }
    ...
    detachedNodes.forEach(function (node) {
        var detachedFromComponent = node.au_;
