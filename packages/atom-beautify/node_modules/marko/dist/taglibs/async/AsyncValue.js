...
AsyncValue.prototype = {
     ...
     * available.
     */
    bB_: function (callback) {
        // Do we already have data or error?
        if (this.bA_) {
            // invoke the callback immediately
            return callback(this.by_, this.aA_);
        }

        var callbacks = this.bz_ || (this.bz_ = []);
    ...
    bD_: function (value) {
        ...
        if (value && typeof value.then === "function") {
            ...
            var finalPromise = value.then(function onFulfilled(value) {
                nextTick(asyncValue.bD_.bind(asyncValue, value));
            }, function onRejected(err) {
                nextTick(asyncValue.bC_.bind(asyncValue, err));
            });
